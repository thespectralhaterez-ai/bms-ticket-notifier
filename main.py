    ...
    for vname, vshows in venue_groups.items():
        ...
        for s in vshows:
            ...
            cats = " | ".join(
                f"{escape(c.name)} "
                f"Rs.{escape(c.price)} "
                f"({_cat_status_label(c.status)})"
                for c in s.categories
            )

            fmt = (
                f" [{escape(s.screen_attr)}]"
                if s.screen_attr
                else ""
            )
...
def main():
    ...
    if not event_code or not region_slug:
        ...
        print(
            "  ❌ Invalid BMS_URL. "
            "Could not extract event/region."
        )
